...
describe('transformColorKeyToCss', () => {
    ...
    describe('Success operation', () => {
      ...
      it('should include forced disabled selector gated by activator (inline)', () => {
        ...
        expect(result).toEqual(
          '.abc.-d.-a { background-color: #4040bf80 }'
        );
      });
    });
  ...
  describe('reference (== styleKeys)', () => {
    describe('Success operation', () => {
      ...
      it('should combine compound states on the parent for reference keys when forceState is true', () => {
        const result = transformColorKeyToCss('boxColor==selected:hover__[240,50,50,0.5]', className, true);
        // parent gets activator always; selected forced class (.-s) and hover native/forced variations
        expect(result).toEqual('.-a:hover.-s .abc, .-a.-s.-h .abc { background-color: #4040bf80 }');
      });
